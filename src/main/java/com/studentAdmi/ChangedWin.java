...
public class ChangedWin extends JFrame{
    ...
    public ChangedWin(String name, String sex, String place, String code, String dept, String ban) {
        ...
        try {
            studentMessage = JdbcAction.findStudent(codeText.getText());
        ...
        OKButton.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
        		 if (nameText.getText().length() == 0 || nameText.getText().contains(" ")) {
                     ...
                     try {
                         int num = JdbcAction.changeStudent(nameText.getText(), sexText.getText(), placeText.getText(), codeText.getText(), deptText.getText(), banText.getText());
                         if (num == 1){        //返回1说明有修改成功
                             JOptionPane.showMessageDialog(null, "修改完成！", "提示",JOptionPane.WARNING_MESSAGE);   //弹出提示框
                            if (Arrays.equals(studentMessage, JdbcAction.findStudent(codeText.getText()))){
                                logger.info("CHANGE【" + nameText.getText() + " : " +codeText.getText() + "】" + "修改了一个记录！但没有什么变化！");
                            }else {
                                logger.info("CHANGE【" + nameText.getText() + " : " +codeText.getText() + "】" + "修改了一个记录！");
                                studentMessage = JdbcAction.findStudent(codeText.getText());
                            }
                         }
                     } catch (SQLException sqlException) {
